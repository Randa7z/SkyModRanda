...
gg.alert("Script Modifikasi by ⚠️RANDA⚠️")

menu = gg.choice({"🔑MULAI🔑"},nil,"🔧MENJALANKAN SCRIPT🔧")
...
yellow = {
    ...
    },
    {
        "🔧PENGATURAN UI🔧",
        content = {
            "🔧IKON DAN BORDER CHAT SIZE",
            "🔧CHAT TRANSPARAN",
            "👚MEMBUKA AKSESORIS",
            "❤️MEMBUKA PENGATURAN PERTEMANAN ",
            "🪄KEMBANG API TAK TERBATAS",                 
            "📡MODE SOLO",
        },
    },           
    {
        "🛠️PENGATURAN PLAYER🛠️",
        content = {
            "☀️CAHAYA☀️",
        },                                     
    },
    {
        "🛠️MEMINDAHKAN SUBJEK🛠️",
        content = {
            "🕯️LILIN",
            "🙎NPC",
        },
...
function farmMenu()
    ...
    getLevel_Cords()
    local content = {
        " | [🕯]JALANKAN LILIN OTOMATIS",
          " | [⭐]JALANKAN BINTANG OTOMATIS",
            " | [🕯]MANUAL CR",
              " | [⭐]MANUAL WL",
                " | [🌋]BANTUAN EDEN",
                  " | [🔖]BUKA QUEST MUSIMAN",
                    " | [ℹ️]MENJALANKAN INSTRUKSI",        
    };
...
end dontRemove = "Script Modifikasi by RANDA";
function hold()
      ...
      do
        SkidLock = skidLevel statue = {}
         statue.limit = #scrSoul[level_Cords].AK_Runner
          statue.trace = 0
         dontRemove = 'Script Modifikasi By Randa'
...
function settings()
    STAY = settings;
    local content = {
        "🛠️OTOMATIS HAPUS ANGIN" .. kj.statSwitch(configs.awrob),
        "🛠️MENEMBUS JARAK: " .. configs.bdis,
        "🛠️MODE DEBUG" .. kj.statSwitch(configs.devmode),
        "🛠️KEMBALI KERUMAH DENGAN CEPAT" .. kj.statSwitch(configs.fasthome),
        "🛠️MEMBAKAR🔥 | MENARIK🕯️ V2" .. kj.statSwitch(configs.mumu),
        "🛠️MODE BETA" .. kj.statSwitch(configs.pbeta),
        "🛠️PENGATURAN RAMUAN" .. configs.sockets,
        "🛠️EXPORT LOKASI📩",
        "🛠️IMPORT LOKASI📩",
        "🛠️RESET PENGATURAN",
        "🛠️RESET PENGATURAN LOKASI",
        "⚙️INFO SCRIPT",
    };
    content = kj.prepareMenu(content, true);
    local tear = gg.choice(content, nil, 'Script MOD by ☣️RANDA☣️' .. '  ' .. randa);
    if tear == eye[1] then
        yellowTears();
    elseif tear == eye[2] then
    ...
    elseif tear == eye[9] then
        local dir = '/storage/emulated/0/Download/Randa-Locations.txt';
        local file = io.open(dir, 'w');
        file:write([[--Locations file generated by: Randa]] .. '\nlocal ela = true\n');
        file:write('slocs_local = ' .. kj.tableToString(slocs));
        file:close();
        gg.alert('[📤]Output Saved Locations:\nFile name: ' .. 'Randa-Locations.txt' .. '\nDirectory: ' .. '/storage/emulated/0/Download');
    elseif tear == eye[10] then
...
function magicF()
    STAY = magicF;
    local content = {
        "[🔮]SETTING MANTRA SIHIR",
        "[✨]EFFECT MANTRA" .. kj.statSwitch(configs.ssparks),
        "[✨]OTOMATIS SUARA",
        "[🔁]MANTRA CAPE RANDOM",
    };
    content = kj.prepareMenu(content, true);
    table.insert(content, "[❌]HAPUS SEMUA MANTRA");
    local tear = gg.choice(content, nil, header);
    if tear == eye[1] then
        return yellowTears();
    elseif tear == eye[2] then
...
function selectSpell()
    ...
    for i = 1, configs.sockets do
        ...
        if content[1][i] == nil then
            content[1][i] = "  [❌]TIDAK ADA";
        end
        if i < 10 then o = 0; else o = ""; end
        table.insert(msocket, "| [🔮]MANTRA [" .. o .. i .. "]: " .. content[1][i]);
    end
    table.insert(msocket, 1, back[1]);
    table.insert(msocket, 2, "[❌]MENGHAPUS SEMUA");
    ...
    elseif tear ~= nil then
        ...
        table.insert(content[2], 2, "[❌]HAPUS");
        for i, v in ipairs(magics) do
            table.insert(content[2], v[1] .. v[2]);
        end
        tear = gg.choice(content[2], nil, "[🔮]MANTRA [" .. x .. "]  |   " .. content[1][x]);
        if tear == eye[1] then
            return selectSpell();
        elseif tear == eye[2] then
            return setspell(0, x);
...
function selectLevel(bto)
    ...
    if _bto_ == "teleports()" or _bto_ == "yellowStar()" then
      ...
      do
        ...
        do
          do
            table.insert(realms, 1, "[🔄]SEGARKAN PETA")
...
instructions = {
    run = {
        [[
            ...
            ---------------------------------------------
            [🕯]OTOMATIS CANDLE RUN:
            Kumpulkan Lilin dari peta dengan teleportasi dan serap
            lilin dari peta.
        ]],
        [[
            [ℹ️]Menjalankan Instruksi
            ---------------------------------------------
            [⭐]OTOMATIS STAR RUN:
            Kumpulkan bintang dari peta dengan berteleportasi dan menyerap
            bintang dari peta.
        ]],
        [[
            [ℹ️]Menjalankan Instruksi
            ---------------------------------------------
            [🕯]CANDLE RUN:
            Mengumpulkan lilin sendiri atau dengan pemain lain dengan mengubah
            koordinat Anda untuk berteleportasi ke setiap lokasi lilin/tanaman.
            Anda dapat membatalkan teleportasi dengan mengklik ikon gg.
        ...
        [[
            ...
            ---------------------------------------------
            [⭐]STAR RUN:
            Mengumpulkan bintang sendiri atau dengan pemain lain dengan mengubah
            koordinat Anda untuk berteleportasi ke setiap lokasi bintang.
        ]],
        [[
            [ℹ️]Menjalankan Instruksi
            ---------------------------------------------             
    ...
    description = {
        [[
            🔧SCRIPT MODIFIKASI BY ☣️RANDA☣️
            ---------------------------------------------
            TERIMA KASIH TELAH MENGGUNAKAN SCRIPT MOD INI!!!
        ]],
    }
}
function offseter()
...
function vcheck()
    ...
    local Randa = "-[" .. debug.getinfo(launch).lastlinedefined + 1 .. "]-";
    if string.find(package, 'com.tgc.sky.android.test') then
        gg.toast('[Beta]Randa' .. Randa);
    ...
    elseif string.find(package, 'com.tgc.sky.android') then
        gg.toast('MENJALANKAN SCRIPT BY ☣️RANDA☣️' .. randa)
...
function startUpTrigger()
  ...
  if not pcall(loadconfigs) then
      kj.post({
    [[[☣️]KONFIGURASI ANDA RUSAK.
    ---------------------------------------------
      SCRIPT AKAN MENGATUR ULANG MEREKA
      KONFIGURASI LAMA ANDA AKAN DISIMPAN KE FOLDER UNDUHAN ANDA.
      ANDA DAPAT MENEMUKAN LOKASI TERSIMPAN ANDA DI SANA DAN MELETAKKANNYA
      KEMBALI SECARA MANUAL DI KONFIGURASI ANDA.
    ]]})
      local hotLoad = io.open('/sdcard/Randa.kj', 'r')
      hotLoad = hotLoad:read("*a")
      local hotSave = io.open('/storage/emulated/0/Download/Randa-Corrupted Settings.txt', 'w')
